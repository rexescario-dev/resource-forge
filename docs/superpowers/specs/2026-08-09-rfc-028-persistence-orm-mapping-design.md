...
**Date:** 2026-08-09  
**Status:** Accepted  
**M3:** Accepted (2026-08-09) — Design Review; no design blockers. Locks Resource-authoritative, one-way, total semantic correspondence ledger over existing floors (identity / Fields / Relations) with RFC-024–027 consumed as mapping inputs. Correspondence ≠ particular storage mechanism; Honor ≠ implement; type identity ≠ instance PK; logical ≠ physical names. No new declaration members, mapping descriptor, adapter port, or public checker; `validateResource`, `evaluateCascadeEvent`, and `checkRelationLoadStates` unchanged and unwired. Prisma/SQL/ORM realization, reverse/sync, runtime traversal/query, Relation→metadata projection, wire, and constraint/index projection deferred. RFC-024–027 / M3.21–M3.24 closed. M4 authorized (planning may be docs-only if no host-independent core surface is required; M4 Integrations realize the correspondence).  
...
**Followed by:** Optional M3.x planning/delivery after Accept ([#98](https://github.com/rexescario-dev/resource-forge/issues/98) or successor) only if a host-independent core representation is required; otherwise M4 Integrations (e.g. Prisma) realize the correspondence; runtime traversal / query execution; Relation→metadata projection; wire/serialization (unless a future RFC proves a hard contract boundary)  
**Unblocks:** A stable core persistence-correspondence contract for M4 Integrations to implement against; optional M3.x planning if core delivery artifacts are proven necessary  
