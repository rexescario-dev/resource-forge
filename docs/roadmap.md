...
| M3 | Resource model | In progress — M3.1 ✅; M3.2 ✅; RFC-006 Annotations Draft ([#8](https://github.com/rexescario-dev/resource-forge/issues/8)); M3.3+ blocked until RFC-006 Accepted |
...
| RFC-006 | Annotations | Draft — [#8](https://github.com/rexescario-dev/resource-forge/issues/8) |
...
**Status:** In progress — [M3 implementation plan](superpowers/plans/2026-08-07-m3-implementation-plan.md) Accepted; [M3.1 task plan](superpowers/plans/2026-08-07-m3-1-resource-contracts.md) Accepted (shipped). [M3.2 projection task plan](superpowers/plans/2026-08-08-m3-2-projection.md) Accepted (implementation complete). Next: [RFC-006 Annotations](superpowers/specs/2026-08-08-rfc-006-annotations-design.md) Draft ([#8](https://github.com/rexescario-dev/resource-forge/issues/8)) → M3 Design Review; M3.3+ implementation remains blocked until RFC-006 is Accepted (and later schema RFCs as applicable).

RFC-005 defines the authoritative Resource aggregate (`identity`, `schema`, `annotations`) and one-way projection to `ResourceMetadata`. Annotation container contracts are Draft in RFC-006; named annotation vocabulary and schema member vocabulary remain later RFCs.
...
- **M3.3+** — deferred until RFC-006 is Accepted and Resource Fields / Relations / Operations RFCs exist as needed
