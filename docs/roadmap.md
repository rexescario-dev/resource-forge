...
| M5 | CLI & end-to-end examples | In progress — M5.1 CLI Foundation ✅ · M5.2 validate ✅ · M5.3 doctor ✅ · M5.4 generate resource ✅ · M5.5 init ✅ · M5.6 from-prisma ✅ · M5.7 examples/basic ✅ · [#121](https://github.com/rexescario-dev/resource-forge/issues/121) · [#124](https://github.com/rexescario-dev/resource-forge/issues/124) · [#128](https://github.com/rexescario-dev/resource-forge/issues/128) · [#132](https://github.com/rexescario-dev/resource-forge/issues/132) · [#135](https://github.com/rexescario-dev/resource-forge/issues/135) · [#138](https://github.com/rexescario-dev/resource-forge/issues/138) · [#141](https://github.com/rexescario-dev/resource-forge/issues/141) |
...
6. **M5.6 CLI Generate from Prisma** — `rf generate from-prisma <dmmfPath> <outDir> --namespace <ns>` (RFC-041) ✅ — [#138](https://github.com/rexescario-dev/resource-forge/issues/138) / [#139](https://github.com/rexescario-dev/resource-forge/pull/139)
7. **M5.7 Basic CLI Examples Walkthrough** — `examples/basic` CLI lifecycle + harness (RFC-042) ✅ — [#141](https://github.com/rexescario-dev/resource-forge/issues/141) / [#142](https://github.com/rexescario-dev/resource-forge/pull/142)
8. **Later slices (not committed APIs)** — other generate kinds, host-application examples, project/workspace doctor, and further Prisma DX only after their own Accepted designs

Roadmap candidates that are **not** committed APIs until Accepted:

```text
(other generate kinds / host examples / project doctor / further Prisma DX)
```

`rf validate` is locked by Accepted RFC-037 (`#124`). `rf doctor` is locked by Accepted RFC-038 (`#128`). `rf generate resource` is locked by Accepted RFC-039 (`#132`). `rf init` is locked by Accepted RFC-040 (`#135`). `rf generate from-prisma` is locked by Accepted RFC-041 (`#138`). `examples/basic` is locked by Accepted RFC-042 (`#141`) as a non-authoritative CLI walkthrough.

Examples demonstrate CLI usage (and later, end-to-end hosts). They are not the source of truth for architecture.
