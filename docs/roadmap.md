...
7. **Later slices (not committed APIs)** — other generate kinds, examples, project/workspace doctor, and further Prisma DX only after their own Accepted designs

Roadmap candidates that are **not** committed APIs until Accepted:

```text
(other generate kinds / project doctor / examples)
```

`rf validate` is locked by Accepted RFC-037 (`#124`). `rf doctor` is locked by Accepted RFC-038 (`#128`). `rf generate resource` is locked by Accepted RFC-039 (`#132`). `rf init` is locked by Accepted RFC-040 (`#135`). `rf generate from-prisma` is locked by Accepted RFC-041 (`#138`).
